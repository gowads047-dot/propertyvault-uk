...
/**
 * The guaranteed-rent share card. One renderer; the main page and each
 * city page have an opengraph-image.tsx of a few lines that names the
 * pill — "For Landlords", "For Landlords in Nottingham" — so a landlord
 * forwarded the Nottingham page sees Nottingham on the card. Colours are
 * literals: next/og cannot read CSS variables.
 */
export function guaranteedRentCard(pill: string, alt: string) {
  return {
    alt,
    render: () => renderCard(pill),
  };
}

function renderCard(pill: string) {
    ...
    (
      <div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", background: "linear-gradient(160deg, #050912 0%, #0f1b36 40%, #1a2e5a 100%)", padding: "60px 80px", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div style={{ padding: "8px 20px", borderRadius: "20px", background: "rgba(244,211,94,0.15)", color: "#f4d35e", fontSize: "16px", fontWeight: 700, alignSelf: "flex-start" }}>{pill}</div>
