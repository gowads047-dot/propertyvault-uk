...
export interface QueueHealth {
  ...
  gapsNext14: string[];
  /** Days in the window with a live row: 14 minus the gaps. */
  daysCovered: number;
...
export async function queueHealth(deps: { db: SocialStore; now: Date; channel?: string }): Promise<QueueHealth> {
  ...
  const [paused, live, holds, fails, latest, pool, cap] = await Promise.all([
    ...
    // Only the newest. Reading every published row to keep one grows with
    // the campaign for no reason.
    db.findPosts({ channel, status: ["published"], orderBy: "published_at", direction: "desc", limit: 1 }),
  ...
  return {
    ...
    gapsNext14: gaps,
    daysCovered: HORIZON_DAYS - gaps.length,
