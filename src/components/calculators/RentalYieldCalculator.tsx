      ...
      <div className="space-y-5">
        <div>
          ...
          <div className="relative">
            ...
            <input aria-label="Property Purchase Price" type="number" min={0} step={5000} value={propertyPrice} onChange={e => setPropertyPrice(nonNegative(e.target.value))}
        ...
        <div>
          ...
          <div className="relative">
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-navy-400">£</span>
            <input aria-label="Annual Running Costs (insurance, maintenance, etc.)" type="number" min={0} step={100} value={annualCosts} onChange={e => setAnnualCosts(nonNegative(e.target.value))}
              className="w-full pl-8 pr-4 py-3 border border-navy-200 rounded-lg text-navy-800 font-semibold focus:outline-none focus:ring-2 focus:ring-gold-400" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-semibold text-navy-700 mb-1">Management Fee ({managementFee}%)</label>
          <input aria-label="Management Fee" type="range" min={0} max={20} step={1} value={managementFee} onChange={e => setManagementFee(nonNegative(e.target.value))} className="w-full accent-gold-500" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-navy-700 mb-1">Void Weeks Per Year ({voidWeeks})</label>
          <input aria-label="Void Weeks Per Year" type="range" min={0} max={8} step={1} value={voidWeeks} onChange={e => setVoidWeeks(nonNegative(e.target.value))} className="w-full accent-gold-500" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-navy-700 mb-1">Monthly Mortgage Payment</label>
          <div className="relative">
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-navy-400">£</span>
            <input aria-label="Monthly Mortgage Payment" type="number" min={0} step={25} value={mortgagePayment} onChange={e => setMortgagePayment(nonNegative(e.target.value))}
              className="w-full pl-8 pr-4 py-3 border border-navy-200 rounded-lg text-navy-800 font-semibold focus:outline-none focus:ring-2 focus:ring-gold-400" />
          </div>
        </div>
      </div>
