        ...
        <div className="container-max">
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="space-y-4">
              <div><label className="block text-sm font-semibold text-navy-700 mb-1">Loan Amount</label><div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-navy-400">£</span><input aria-label="Loan Amount" type="number" min="0" value={loanAmount} onChange={e => setLoanAmount(nonNegative(e.target.value))} className="w-full pl-8 pr-4 py-3 border border-navy-200 rounded-lg font-semibold focus:outline-none focus:ring-2 focus:ring-gold-400" /></div></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-semibold text-navy-700 mb-1">Monthly Rate ({monthlyRate}%)</label><input aria-label="Monthly Rate" type="range" min={0.4} max={2} step={0.05} value={monthlyRate} onChange={e => setMonthlyRate(nonNegative(e.target.value))} className="w-full accent-gold-500" /><p className="text-xs text-navy-500 text-center">{results.annualRate.toFixed(1)}% annual equivalent</p></div>
                <div><label className="block text-sm font-semibold text-navy-700 mb-1">Term ({termMonths} months)</label><input aria-label="Term ( months)" type="range" min={1} max={24} step={1} value={termMonths} onChange={e => setTermMonths(nonNegative(e.target.value))} className="w-full accent-gold-500" /></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs font-semibold text-navy-700 mb-1">Valuation Fee (£)</label><input aria-label="Valuation Fee (£)" type="number" min="0" value={valuationFee} onChange={e => setValuationFee(nonNegative(e.target.value))} className="w-full px-3 py-2 border border-navy-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400" /></div>
                <div><label className="block text-xs font-semibold text-navy-700 mb-1">Legal Fees (£)</label><input aria-label="Legal Fees (£)" type="number" min="0" value={legalFees} onChange={e => setLegalFees(nonNegative(e.target.value))} className="w-full px-3 py-2 border border-navy-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400" /></div>
              </div>
            </div>
