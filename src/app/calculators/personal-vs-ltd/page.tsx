        ...
        <div className="container-max">
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <h2 className="font-bold text-navy-800">Property Details</h2>
              <div><label className="block text-xs font-semibold text-navy-700 mb-1">Annual Rental Income</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-navy-400 text-sm">£</span><input aria-label="Annual Rental Income" type="number" min="0" value={rentalIncome} onChange={e => setRentalIncome(nonNegative(e.target.value))} className="w-full pl-7 pr-3 py-2.5 border border-navy-200 rounded-lg text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-gold-400" /></div></div>
              <div><label className="block text-xs font-semibold text-navy-700 mb-1">Annual Mortgage Interest</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-navy-400 text-sm">£</span><input aria-label="Annual Mortgage Interest" type="number" min="0" value={mortgageInterest} onChange={e => setMortgageInterest(nonNegative(e.target.value))} className="w-full pl-7 pr-3 py-2.5 border border-navy-200 rounded-lg text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-gold-400" /></div></div>
              <div><label className="block text-xs font-semibold text-navy-700 mb-1">Other Allowable Expenses</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-navy-400 text-sm">£</span><input aria-label="Other Allowable Expenses" type="number" min="0" value={expenses} onChange={e => setExpenses(nonNegative(e.target.value))} className="w-full pl-7 pr-3 py-2.5 border border-navy-200 rounded-lg text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-gold-400" /></div></div>

              <div className="border-t border-navy-100 pt-4">
                <h2 className="font-bold text-navy-800 mb-3">Your Tax Position</h2>
                <div><label className="block text-xs font-semibold text-navy-700 mb-1">Other Annual Income <span className="font-normal text-navy-400">(salary, pension, etc.)</span></label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-navy-400 text-sm">£</span><input aria-label="Other Annual Income (salary, pension, etc.)" type="number" min="0" value={otherIncome} onChange={e => setOtherIncome(nonNegative(e.target.value))} className="w-full pl-7 pr-3 py-2.5 border border-navy-200 rounded-lg text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-gold-400" /></div><p className="text-xs text-navy-400 mt-1">Used to compute your marginal tax band and Section 24 credit</p></div>
                <div className="mt-3"><label className="block text-xs font-semibold text-navy-700 mb-1">Associated Companies <span className="font-normal text-navy-400">(other group companies)</span></label><input aria-label="Associated Companies (other group companies)" type="number" min={0} max={9} value={associatedCompanies} onChange={e => setAssociatedCompanies(nonNegative(e.target.value))} className="w-full px-3 py-2.5 border border-navy-200 rounded-lg text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-gold-400" /><p className="text-xs text-navy-400 mt-1">Divides the CT thresholds (0 for a standalone SPV)</p></div>
              </div>
            ...
            <div className="bg-white rounded-xl border border-navy-100 p-5">
              <h2 className="font-bold text-navy-800 mb-1">Personal Ownership</h2>
            ...
            <div className={`rounded-xl border-2 p-5 ${results.ltdBetter ? "border-green-400 bg-green-50/30" : "border-navy-100 bg-white"}`}>
              <h2 className="font-bold text-navy-800 mb-1">Limited Company (SPV)</h2>
