          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-semibold text-navy-700 mb-1">Current Rate ({currentRate}%)</label><input aria-label="Current Rate" type="range" min={1} max={10} step={0.1} value={currentRate} onChange={e => setCurrentRate(nonNegative(e.target.value))} className="w-full accent-gold-500" /></div>
                <div><label className="block text-sm font-semibold text-navy-700 mb-1">New Rate ({newRate}%)</label><input aria-label="New Rate" type="range" min={1} max={10} step={0.1} value={newRate} onChange={e => setNewRate(nonNegative(e.target.value))} className="w-full accent-gold-500" /></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs font-semibold text-navy-700 mb-1">Legal Fees</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-navy-400 text-sm">£</span><input aria-label="Legal Fees" type="number" min="0" value={legalFees} onChange={e => setLegalFees(nonNegative(e.target.value))} className="w-full pl-7 pr-2 py-2 border border-navy-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400" /></div></div>
                <div><label className="block text-xs font-semibold text-navy-700 mb-1">Early Exit Fee</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-navy-400 text-sm">£</span><input aria-label="Early Exit Fee" type="number" min="0" value={exitFee} onChange={e => setExitFee(nonNegative(e.target.value))} className="w-full pl-7 pr-2 py-2 border border-navy-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold-400" /></div></div>
              </div>
            </div>
