...
const faqs = [
  ...
  { q: "What happens if my rental property doesn't meet the EPC requirement?", a: "Under the confirmed rules, landlords could face fines of up to £30,000 per property for non-compliance. An exemption register exists for properties where the £10,000 cost cap has been reached but EPC C is still not achievable, or where improvements would damage the building's structure — confirm the exact spend cap figure on GOV.UK." },
];
    ...
    <>
                  ...
                  {measures.map((m) => (
                    <div key={m.id} className={`rounded-xl border p-4 transition-all ${m.enabled ? "border-gold-400 bg-gold-50/30" : "border-navy-100 bg-white"}`}>
                      <div className="flex items-start gap-3">
                        <button onClick={() => toggleMeasure(m.id)} role="checkbox" aria-checked={m.enabled} aria-label={m.name}
      ...
      <section className="section-padding bg-white">
        <div className="container-max max-w-3xl space-y-6 text-navy-700 text-sm leading-relaxed">
          ...
          <p>Under the rules set in the government&apos;s <a href="https://www.gov.uk/government/consultations/improving-the-energy-performance-of-privately-rented-homes-2025-update/outcome/improving-the-energy-performance-of-privately-rented-homes-government-response-html" target="_blank" rel="noopener noreferrer" className="text-gold-600 font-semibold">January 2026 response</a>, the most a landlord is required to spend to reach EPC C is £10,000 per property (the consultation had proposed £15,000). Spend up to this and still can&apos;t reach C? Register for an exemption, which then lasts ten years. Exemptions also apply where improvements would damage the property's structure — common with listed buildings and older stone properties.</p>
