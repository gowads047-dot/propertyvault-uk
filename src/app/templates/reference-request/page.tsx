          ...
          <div className="container-max max-w-2xl space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-navy-100 p-6 space-y-4">
              ...
              <div><label className="block text-sm font-semibold text-navy-700 mb-1">{type === "employer" ? "Company / Organisation" : "Property Management Company / Agency"}</label>
                <input aria-label={type === "employer" ? "Company / Organisation" : "Property Management Company / Agency"} value={refereeCompany} onChange={e => setRefereeCompany(e.target.value)} placeholder={type === "employer" ? "Employer company name" : "Landlord / agency name"} className="w-full px-4 py-2.5 border border-navy-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-gold-400" /></div>
