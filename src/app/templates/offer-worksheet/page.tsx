        ...
        <section style={{ background: "#e8ecf0", padding: "32px 16px 64px" }}>
          ...
          <div id="print-doc" style={{ maxWidth: 800, margin: "0 auto", background: "white", boxShadow: "0 4px 40px rgba(0,0,0,0.15)", fontFamily: "Arial, sans-serif", color: "#1a1a1a" }}>
            ...
            <div style={{ padding: "28px 36px 36px" }}>
              <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0f1b36", marginBottom: 4 }}>Offer Preparation Worksheet</h1>
              <p style={{ fontSize: 11, color: "#64748b", marginBottom: 20 }}>Property research, comparable analysis, and offer strategy</p>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginTop: 28 }}>
                <div style={{ border: "1px solid #e2e8f0", padding: 14 }}>
                  <p style={{ fontSize: 10, fontWeight: 700, color: "#0f1b36", marginBottom: 20 }}>OFFER SUBMITTED BY</p>
                  <div style={{ borderBottom: "1px solid #1a1a1a", marginBottom: 4 }} />
                  <p style={{ fontSize: 9, color: "#64748b" }}>{buyerName || "Name: ___________________________"}</p>
                  <p style={{ fontSize: 9, color: "#64748b", marginTop: 4 }}>Date: {offerDate ? new Date(offerDate).toLocaleDateString("en-GB") : "___________________________"}</p>
                </div>
                <div style={{ border: "1px solid #e2e8f0", padding: 14, background: "#f8f9fc" }}>
                  <p style={{ fontSize: 10, fontWeight: 700, color: "#0f1b36", marginBottom: 8 }}>OFFER SUMMARY</p>
                  <p style={{ fontSize: 24, fontWeight: 900, color: "#0f1b36" }}>{fmt(offerPrice)}</p>
                  <p style={{ fontSize: 11, color: "#64748b" }}>{offerPct}% of asking · saving {fmt(saving)}</p>
                  {avgComp > 0 && <p style={{ fontSize: 10, color: "#94a3b8", marginTop: 4 }}>Comps avg: {fmt(avgComp)}</p>}
                </div>
              </div>

              <p style={{ fontSize: 8, color: "#94a3b8", marginTop: 24, lineHeight: 1.5 }}>
                This worksheet is for personal research purposes only. Not financial or legal advice. Comparable prices sourced by the user from publicly available data. Generated by PropertyVault UK. Ref: {refNum}. © {new Date().getFullYear()} PropertyVault UK.
              </p>
            </div>
          </div>
        </section>
...
function DocSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginTop: 20 }}>
      <div style={{ background: "#0f1b36", padding: "5px 12px", display: "inline-block", marginBottom: 0 }}>
        <p style={{ fontSize: 10, fontWeight: 700, color: "white", textTransform: "uppercase", letterSpacing: "0.05em" }}>{title}</p>
      </div>
