  ...
  return (
    <div style={{ background: "#0a0f1e", minHeight: "100vh", color: "white" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.62)", fontSize: 14, marginBottom: 32 }}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.58)", marginBottom: 20 }}>Step-by-step written guides — video versions coming soon.</p>
            ...
            {RESOURCES.map(r => (
              <Link key={r.title} href={r.href} style={{ textDecoration: "none", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "18px 20px", display: "block", transition: "border-color 0.2s" }}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,0.62)" }}>{r.duration}</span>
                </div>
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.58)", marginBottom: 20 }}>Video and audio content is in production. Members will be notified first.</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 14 }}>
            {COMING_SOON.map(item => (
              <div key={item.title} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 20, opacity: 0.7 }}>
                ...
                <p style={{ fontSize: 12, color: "rgba(255,255,255,0.58)", lineHeight: 1.6 }}>{item.desc}</p>
                <div style={{ marginTop: 12, display: "inline-block", fontSize: 10, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.58)" }}>Coming Soon</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginTop: 40, background: "rgba(212,175,55,0.06)", border: "1px solid rgba(212,175,55,0.2)", borderRadius: 16, padding: 24, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.58)", marginBottom: 16 }}>Drop Nass a message and we&apos;ll notify you as soon as the first episode is live.</p>
          <a href="https://wa.me/447415721628?text=Hi%20Nass%2C%20I%27m%20an%20Academy%20member%20and%20I%27d%20like%20to%20be%20notified%20when%20the%20video%20content%20goes%20live!" target="_blank" rel="noopener noreferrer"
            style={{ display: "inline-block", background: "#25D366", color: "#0b2e18", fontWeight: 700, fontSize: 13, padding: "10px 24px", borderRadius: 12, textDecoration: "none" }}>
            WhatsApp Nass →
          </a>
        </div>
      </div>
